...
/*    private GlobalVariableElement getAndRegisterGlobalArray(ExecutableElement originalElement, GlobalVariableElement globalArray) {
        Assert.assertNotNull(globalArray);
        if (originalElement != null && !globalArray.getEnclosingType().equals(originalElement.getEnclosingType())) {
            Section section = ctxt.getImplicitSection(originalElement.getEnclosingType());
            section.declareData(null, globalArray.getName(), globalArray.getType());
        }
        return globalArray;
    }

    public GlobalVariableElement getAndRegisterGlobalMethodInfoArray(ExecutableElement originalElement) {
        return getAndRegisterGlobalArray(originalElement, methodInfoTable);
    }

    public GlobalVariableElement getAndRegisterGlobalSourceCodeInfoArray(ExecutableElement originalElement) {
        return getAndRegisterGlobalArray(originalElement, sourceCodeInfoTable);
    }

    public GlobalVariableElement getAndRegisterGlobalSourceCodeIndexArray(ExecutableElement originalElement) {
        return getAndRegisterGlobalArray(originalElement, sourceCodeIndexList);
    }

    public GlobalVariableElement getAndRegisterGlobalInstructionArray(ExecutableElement originalElement) {
        return getAndRegisterGlobalArray(originalElement, instructionList);
    }

    public GlobalVariableElement getAndRegisterGlobalInstructionArraySize(ExecutableElement originalElement) {
        return getAndRegisterGlobalArray(originalElement, instructionListSize);
    }

    public GlobalVariableElement getAndRegisterGlobalMethodData(ExecutableElement originalElement) {
        return getAndRegisterGlobalArray(originalElement, methodDataHolder);
    }*/
